...
salt_0 = storar[:,:,0,:,:]; salt_500 = storar[:,:,19,:,:]; salt_5000 = storar[:,:,28,:,:]
salt_5000_mn = np.nanmean(np.nanmean(np.nanmean(salt_5000, axis = 1), axis = 1), axis = 1)
salt_500_mn = np.nanmean(np.nanmean(np.nanmean(salt_500, axis = 1), axis = 1), axis = 1)
salt_0_mn = np.nanmean(np.nanmean(np.nanmean(salt_0, axis = 1), axis = 1), axis = 1)
del storar

var = 'votemper'
storar = ut.extract_model_from_daily_3D(yrstart,yrend,path_to_files,ftype,var)
temp_0 = storar[:,:,0,:,:]; temp_500 = storar[:,:,19,:,:]; temp_5000 = storar[:,:,28,:,:]
temp_5000_mn = np.nanmean(np.nanmean(np.nanmean(temp_5000, axis = 1), axis = 1), axis = 1)
temp_500_mn = np.nanmean(np.nanmean(np.nanmean(temp_500, axis = 1), axis = 1), axis = 1)
...
var = 'DIC'
storar = ut.extract_model_from_daily_3D(yrstart,yrend,path_to_files,ftype,var)
DIC_0 = storar[:,:,0,:,:]; DIC_500 = storar[:,:,19,:,:]*1e6; DIC_5000 = storar[:,:,28,:,:]*1e6
DIC_5000_mn = np.nanmean(np.nanmean(np.nanmean(DIC_5000, axis = 1), axis = 1), axis = 1)
DIC_500_mn = np.nanmean(np.nanmean(np.nanmean(DIC_500, axis = 1), axis = 1), axis = 1)
...
axs[1].plot(yrs, salt_500_mn, 'r:', label = '500m')
axs[1].plot(yrs, salt_5000_mn, 'r--', label = '5000m')
axs[1].set_title('Mean model salinity (3 depths)')

axs[2].plot(yrs, temp_0_mn, 'r-', label = '0m')
axs[2].plot(yrs, temp_500_mn, 'r:', label = '500m')
axs[2].plot(yrs, temp_5000_mn, 'r--', label = '5000m')
axs[2].set_title('Mean model temperature (3 depths)')

axs[3].plot(yrs, DIC_0_mn, 'k-', label = '0m')
axs[3].plot(yrs, DIC_500_mn, 'k:', label = '500m')
axs[3].plot(yrs, DIC_5000_mn, 'k--', label = '5000m')
