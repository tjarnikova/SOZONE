...
def  make_yearly_subset(yr,runname, dtype = 'ptrc-T', tdir = 'ukesm_allscen_ptrcT_ferphd'):

    #what is new file going to be called:
    savenam = f'/gpfs/data/greenocean/software/resources/MEDUSA/PROC2/medusa_{runname}_1y_{yr}_ptrc-T-FERPHD.nc'
    print(savenam)
    ...
    if len(fils) != 12:
        ...
        FER = np.zeros([12,75,332,362])
        PHD = np.zeros([12,75,332,362])
#         CHD = np.zeros([12,75,332,362])
#         CHN = np.zeros([12,75,332,362])
        
        for i in range(0,12):
            tfil = xr.open_dataset(fils[i])
            FER[i,:,:,:] = tfil['FER'][:,:,:].values
            PHD[i,:,:,:] = tfil['PHD'][:,:,:].values
...
#             CHN[i,:,:,:] = tfil['CHN'][:,:,:].values
        ...
        data_vars = {'FER':(['time_counter','deptht','y', 'x'], FER,
                                 {'units': 'mmol-Fe/m3',
                                  'long_name':''}),
                     'PHD':(['time_counter','deptht', 'y', 'x'], PHD,
                                 {'units': 'mmol-N/m3',
                                  'long_name':''}),   
...
#                                   'long_name':''}), 
        ...
        attrs = {'made in':'SOZONE/MEDUSA/MEDUSA_PROCESSING/runner-ptrcT-FERPHD.py',
