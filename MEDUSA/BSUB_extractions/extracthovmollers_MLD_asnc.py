...
def get_hovmols(ukscen,tomscen,start,end):
    ukesm = xr.open_mfdataset(make_yearlist_ukesm(start,end+1,ukscen, dtype = 'MLD'))
    print(ukesm)
    tom = xr.open_mfdataset(make_yearlist_tom(start,end,'grid_T', baseDir = tomscen))
    tom['time_counter'] = \
    ...
    uknam_fg = f'./EXTRACT/UKESM_{scenUKESM}_{varUKESM}_lathovmoller_{start}_{end}.nc'
    tomnam_fg = f'./EXTRACT/{nameTOM}_{varTOM}_lathovmoller_{start}_{end}.nc'
    ukesm_fg.to_netcdf(uknam_fg)
    tom_fg.to_netcdf(tomnam_fg)
