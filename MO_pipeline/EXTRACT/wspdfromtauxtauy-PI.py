...
def make_wspd(tnam, tdir, yr):
    ...
    fnam_tosave = f'/gpfs/data/greenocean/software/resources/winds_gooddates/UKESM_{tnam}_wspd_{yr}_daily.nc'
...
def make_wspd_ERA(yr, vers = '2022'):

    #get and open
    fnam = f'ERA5_v{vers}_taux_{yr}_daily.nc'
    sdir = '/gpfs/data/greenocean/software/resources/winds_gooddates/'
    taux_dat = xr.open_dataset(f'{sdir}{fnam}')

    fnam = f'ERA5_v{vers}_tauy_{yr}_daily.nc'
    sdir = '/gpfs/data/greenocean/software/resources/winds_gooddates/'
    tauy_dat = xr.open_dataset(f'{sdir}{fnam}')

    fnam_tosave = f'/gpfs/data/greenocean/software/resources/winds_gooddates/ERA5_v{vers}_wspd_{yr}_daily.nc'
...
def runner(tnam, tdir, yrstart, yrend):

    for yr in range(yrstart,yrend):
        make_wspd(tnam, tdir,yr)

    return 

def runner_era(yrstart, yrend, vers):

    for yr in range(yrstart,yrend):
        make_wspd_ERA(yr, vers)

    return 

yrstart = 1959; yrend = 1980
runner_era(yrstart, yrend, vers = '202303')

yrstart = 1950; yrend = 2022
runner_era(yrstart, yrend, vers = '2022')
